...
# ---------------------------------------------------------------------------
# Camera health — services/registry/app/api/routers/health.py
# ---------------------------------------------------------------------------
...
class CameraHealthOut(BaseModel):
    ...
    camera_type: str
    status: str
    # None when no probe has ever run for this camera — a real, honest "we
    # don't know yet", not a fabricated uptime figure. See
    # docs/DATA_HANDLING_AND_RETENTION.md on not inventing numbers.
    last_checked_at: datetime | None = None
    last_probe: str | None = None
    last_is_live: bool | None = None
    last_latency_ms: int | None = None
    last_error_code: str | None = None


# ---------------------------------------------------------------------------
# Incident reports — services/registry/app/api/routers/reports.py
# ---------------------------------------------------------------------------


class IncidentWeekRow(BaseModel):
    jurisdiction_id: int
    jurisdiction_path: str
...
class IncidentReportOut(BaseModel):
    generated_at: datetime
    weeks: int
    rows: list[IncidentWeekRow]


# ---------------------------------------------------------------------------
# Women's-safety risk zones — curated, not official crime data. See
# db/migrations/011_safety_zone.sql and docs/AUTOMATED_PROCESSING_DISCLOSURE.md.
# ---------------------------------------------------------------------------
