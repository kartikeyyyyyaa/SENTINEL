...
box with whatever wheels happened to install.
"""
...
from .primitives import PrimitiveEvent
...
class JsonlTransport:
    ...
    The default when no registry URL is configured, and what the dry run and the
...
class HttpTransport:
    ...
    expected to be idempotent per event id, because a timeout that actually
    succeeded server-side is indistinguishable here from one that did not, and
    re-sending is the only safe response.
    """
    ...
    def send(self, batch: list[PrimitiveEvent]) -> bool:
        body = json.dumps(
            {"events": [e.as_dict() for e in batch]}, separators=(",", ":")
        ).encode("utf-8")
...
class MemoryTransport:
    """Collects batches in a list. Tests, and nothing else.

    ``fail_until`` makes it refuse deliveries for a while, which is how the
    backpressure and drop-oldest behaviour is tested without a network.
    """
...
def build_transport(config: SinkConfig) -> Transport:
    """HTTP when a registry URL is set, JSONL otherwise.

    There is no default registry URL on purpose: a worker that posts somewhere
    because a default was left in a config file is worse than one that prints to a
    terminal and makes you notice.
    """
